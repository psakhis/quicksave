...
local function populate_configure_menu(menu)
	...
	table.insert(menu, {_p('plugin-gunlight', 'On frames'), current_button.on_frames, current_button.on_frames > 1 and 'lr' or 'r'})
	table.insert(menu, {_p('plugin-gunlight', 'Off frames'), current_button.off_frames, current_button.off_frames > 1 and 'lr' or 'r'})
...
local function handle_configure_menu(index, event)
	...
	elseif index == 3 then
		-- On frames
		manager.machine:popmessage(_p('plugin-gunlight', 'Number of frames button will be pressed'))
		if event == 'left' then
	...
	elseif index == 4 then
		-- Off frames
		manager.machine:popmessage(_p('plugin-gunlight', 'Number of frames button will be released'))
		if event == 'left' then
